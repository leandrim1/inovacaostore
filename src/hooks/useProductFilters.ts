...
export function useProductFilters(baseProducts: Product[]) {
  ...
  const filtered = useMemo(() => {
    ...
    return result;
  }, [baseProducts, categories, sizes, colors, minPrice, maxPrice, sort]);

  function updateList(key: "tamanho" | "cor" | "categoria", value: string) {
  ...
  const activeFilterCount = sizes.length + colors.length + categories.length +
