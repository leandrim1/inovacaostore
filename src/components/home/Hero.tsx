...
import { VitrineHero3D } from "./VitrineHero3D";
...
export function Hero() {
  ...
  const { webgl, profundidade } = useExperiencia3D();
  ...
  return (
    ...
    <section ref={secaoRef} data-hero className="relative -mt-16 sm:-mt-20">
      <motion.div
        className="relative flex min-h-[calc(92svh+4rem)] items-end overflow-hidden bg-brand-ink sm:min-h-[calc(95svh+5rem)] lg:min-h-[calc(90vh+5rem)]"
        ...
        style={
          profundidade
            ? {
                scale: palcoEscala,
                rotateX: palcoGiro,
                borderBottomLeftRadius: isMobile ? undefined : palcoRaio,
                borderBottomRightRadius: isMobile ? undefined : palcoRaio,
                transformOrigin: "50% 100%",
              }
            : undefined
        }
      >
        {/* No celular e no tablet a foto começa abaixo do header (é "contain",
            ancorada no topo — o enquadramento continua o de sempre); no
            computador ela passa por baixo do vidro. */}
        <div ref={ref} className="absolute inset-x-0 bottom-0 top-16 sm:top-20 lg:top-0" aria-hidden>
          <AnimatePresence>
            {!carregando && (
            <motion.img
              key={currentSlide.id}
        ...
        {webgl !== "off" && (
          <div
            className="pointer-events-none absolute inset-0 hidden bg-[radial-gradient(ellipse_46%_70%_at_76%_48%,rgba(0,0,0,0.62),transparent_74%)] lg:block"
            aria-hidden
          />
        )}
        <VitrineHero3D secao={secaoRef} />
        {webgl === "off" && profundidade && <PoeiraDeLuz />}

        <motion.div
          className="container-page relative z-10 pb-16 pt-36 sm:pb-24 sm:pt-40"
