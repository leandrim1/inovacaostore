...
import { STORE, buildWhatsAppLink, formatCityState } from "../data/store";
import type { SiteSettings } from "../hooks/useSiteSettings";
import { useSiteSettings } from "../hooks/useSiteSettings";
import NotFoundPage from "./NotFoundPage";

type Policy = { title: string; sections: { heading: string; text: string }[] };

// Recebe as configurações porque o texto de "Retirada na loja" cita o
// endereço, que agora é editável no painel.
function buildPolicies(settings: SiteSettings): Record<string, Policy> {
  return {
    "trocas-e-devolucoes": {
    ...
    entrega: {
      ...
      sections: [
        ...
        },
        {
          ...
          text: `Você também pode retirar seu pedido pessoalmente em nossa loja física, na ${settings.addressStreet}, ${formatCityState(settings)}.`,
        },
      ],
    },
  };
...
export default function PolicyPage() {
  ...
  return (
    <>
      <Seo
        title={policy.title}
        description={`${policy.title} da ${STORE.name}.`}
      />
      <div className="container-page max-w-3xl py-14">
        ...
        <div className="flex flex-col gap-6">
          {policy.sections.map((s) => (
            <div key={s.heading}>
              <h2 className="mb-1.5 font-display text-base tracking-wide">
                {s.heading}
              </h2>
        ...
        <p className="mt-10 text-sm text-neutral-500">
          ...
          <a
            href={buildWhatsAppLink(
              ...
              "Olá! Tenho uma dúvida sobre as políticas da loja.",
            )}
            target="_blank"
            rel="noreferrer"
